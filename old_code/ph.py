...
import warnings
import ot
from data import filt_to_matrix
...
def map_multiscale(spatial,cluster_complex,clusterings,num_domains=0,filt=1,plots="on",order='size',redundant_filter=False):
    ...
    for n in range(len(cocycles)+1):
        if n == 0:
            feature_list = [(set(cocycles[0])-set(cocycles[1]),diagram_0d[0][2])]
        elif n == len(cocycles):
            break
        else:
        ...
        if redundant_filter:
            for guy in domains:
                zz = list(i for i in range(len(z)) if (abs(z[i]-guy[i])<.05 and z[i] < redundant_filter))
                gg = list(i for i in range(len(z)) if z[i] < redundant_filter)
            if len(gg) < 1:
                nontrivial = False
                break
    ...
    if order == 'size':   
        ...
        domains = sorted(domains,key = lambda x : np.max(x)-np.min(x))
    domains = domains[:num_domains]

    #Normalize the coreness values to be between 0 and 1
    for i in range(len(coreness)):
        max = np.max(list(z for z in domains[i]))
        min = np.min(list(z for z in domains[i]))-.00001
        z  = list(1-(domains[i][j]-min)/(max-min+.00001) for j in range(len(domains[i])))
        domains[i] = z
...
#This is broken right now, needs to be fixed
